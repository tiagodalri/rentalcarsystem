...
export async function exportBriefingToPdf({ briefing, contextLabel }: Opts) {
  ...
  const drawChrome = () => {
    ...
    doc.setFontSize(8.5);
    doc.text("Zeus Brain · Briefing Executivo", pageW - marginX, 34, { align: "right" });
    ...
    doc.setFont("helvetica", "normal");
    doc.setFontSize(8);
    doc.text("Confidencial · Zeus Rental Car", marginX, pageH - marginBottom + 34);
  ...
  doc.text("Gerado por Zeus Brain · Inteligência operacional para a frota Zeus.", marginX, y);

  // Save
  const fileName = `Zeus-Brain-Briefing-${format(new Date(), "yyyy-MM-dd-HHmm")}.pdf`;
  doc.save(fileName);
}
