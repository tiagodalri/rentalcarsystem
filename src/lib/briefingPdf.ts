...
export async function exportBriefingToPdf({ briefing, contextLabel }: Opts) {
  ...
  const drawChrome = () => {
    ...
    doc.setFont("helvetica", "normal");
    doc.setFontSize(8);
    doc.text("Confidencial · GoDrive", marginX, pageH - marginBottom + 34);
  ...
  doc.text("Gerado por AI Studio · Inteligência operacional para a frota GoDrive.", marginX, y);

  // Save
  const fileName = `GoDrive-Brain-Briefing-${format(new Date(), "yyyy-MM-dd-HHmm")}.pdf`;
  doc.save(fileName);
}
