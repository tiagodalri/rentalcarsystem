...
export default function FullscreenFab() {
  ...
  return (
    <button
      ...
      className="fixed bottom-3 right-3 z-[60] h-9 w-9 rounded-full border border-border/60 bg-background/70 text-muted-foreground shadow-sm backdrop-blur-md transition-all hover:bg-background hover:text-foreground hover:shadow-md opacity-60 hover:opacity-100 print:hidden"
    >
      {isFs ? <Minimize2 className="h-4 w-4 mx-auto" /> : <Maximize2 className="h-4 w-4 mx-auto" />}
    </button>
  );
}
