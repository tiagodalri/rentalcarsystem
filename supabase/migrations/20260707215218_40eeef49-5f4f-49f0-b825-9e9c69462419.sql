...
DECLARE
  ...
  m int; d date; seeded_sum numeric; delta numeric; has_financing boolean; nth int;
BEGIN
  nth := 0;
  FOR r IN (
    WITH pv AS (
    ...
    scored AS (
      SELECT *, (days_hist::numeric/GREATEST(days_owned,1))*100 AS occ,
             ...
             ROW_NUMBER() OVER (ORDER BY (days_hist::numeric/GREATEST(days_owned,1)) ASC) AS rank_asc
      FROM pv
    )
    SELECT * FROM scored
  ) LOOP
    ...
    ELSIF r.q = 4 THEN
      tier := 'carroco';
      -- 2 piores levam mais custo -> margem negativa; os outros ficam ~0
      IF r.rank_asc <= 2 THEN target_ratio := 1.18; ELSE target_ratio := 1.05; END IF;
      w_dep := 0.28; w_ins := 0.12; w_mnt := 0.26; w_cle := 0.04; w_tol := 0.04; w_fin := 0.10;
    ELSE
    ...
    IF exp_target < r.purchase * 0.05 THEN exp_target := r.purchase * 0.05; END IF;
