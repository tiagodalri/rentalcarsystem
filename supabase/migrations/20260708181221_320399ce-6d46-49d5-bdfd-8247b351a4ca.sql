...
-- Indexes to accelerate soft-delete/restore in demo_start_presentation / demo_stop_presentation
...
CREATE INDEX IF NOT EXISTS idx_financial_transactions_vehicle_id ON public.financial_transactions(vehicle_id);
...
-- Re-create demo_start_presentation with statement_timeout as the FIRST instruction,
-- keeping the exact same tiered selection logic and behavior.
...
DECLARE
  ...
  v_n_meio  int;
BEGIN
  -- Primeira instrução: rede de segurança contra statement_timeout global.
